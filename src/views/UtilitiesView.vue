...
function syncFromRoute() {
  ...
  if (path.startsWith('/utilities/herramientas')) {
    ...
    // Default '/utilities' or '/utilities/' -> Pro Hub Home
...
function navigateToTrackingApp() {
  router.push('/')
}

function handleTabClick(tab: 'home' | 'pacientes' | 'utilities') {
  if (tab === 'home') {
    navigateToHome()
...
<template>
  <div class="min-h-screen flex flex-col transition-colors duration-200" style="background: var(--surface-container-lowest); color: var(--on-surface);">
    ...
    <header class="sticky top-0 z-40 backdrop-blur-xl border-b border-slate-200 dark:border-white/10 bg-white/90 dark:bg-[#0e0e10]/90 transition-colors no-print">
      <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
        
        <!-- Main Header Row (Full bar on desktop, Row 1 on mobile) -->
        <div class="h-14 sm:h-16 flex items-center justify-between gap-2 sm:gap-4">
          
          <!-- Brand & Context (Clickable -> goes to utilities home) -->
          <button
            @click="handleTabClick('home')"
            class="flex items-center gap-2.5 sm:gap-3 text-left group cursor-pointer focus:outline-none transition-transform active:scale-95 shrink-0"
            title="Ir al Home de Utilities"
          >
            <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center font-black text-black shadow-md sm:shadow-lg transition-transform group-hover:scale-105 shrink-0" style="background: var(--kinetic-glow);">
              <Wrench class="w-4 h-4 sm:w-5 sm:h-5" />
            </div>
            <div class="min-w-0">
              <div class="flex items-center gap-1.5 sm:gap-2">
                <h1 class="font-bold text-sm sm:text-base tracking-wide text-slate-900 dark:text-white truncate" style="font-family: var(--font-display);">BodyFlow</h1>
                <span class="text-[9px] sm:text-[10px] font-extrabold uppercase px-1.5 sm:px-2 py-0.5 rounded-full shrink-0" style="background: rgba(135, 255, 112, 0.15); color: var(--primary); border: 1px solid rgba(135, 255, 112, 0.3);">
                  PRO HUB
                </span>
              </div>
              <p class="hidden sm:block text-xs text-slate-500 dark:text-slate-400 truncate">Panel de Control y Utilidades</p>
            </div>
          </button>

          <!-- Desktop Center/Right: Navigation Tabs (hidden on mobile, shown sm:) -->
          <nav class="hidden sm:flex items-center gap-1 sm:gap-1.5 p-1 rounded-2xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10">
            <!-- 1. Home -->
            <button
              @click="handleTabClick('home')"
              class="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-200 cursor-pointer"
              :class="[
                currentTab === 'home'
                  ? 'bg-white dark:bg-white/15 text-slate-900 dark:text-white shadow-sm ring-1 ring-slate-200 dark:ring-white/20 font-bold'
              ...
              title="Home del Hub de Utilidades"
            >
              <Home class="w-4 h-4" />
              <span>Home</span>
            </button>
            ...
            <button
              ...
              title="Herramientas y Módulos Pro"
            >
          ...
          <!-- Right Actions: Module Switcher & Theme Toggle -->
          <div class="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <!-- Module Switcher: Ir a App Tracking -->
            <button
              @click="navigateToTrackingApp"
              class="flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-[11px] sm:text-xs font-semibold text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 border border-slate-200 dark:border-white/10 transition-all cursor-pointer shadow-2xs active:scale-95 shrink-0"
              title="Ir a App Tracking (Módulo de Seguimiento)"
            >
              <Smartphone class="w-3.5 h-3.5 text-emerald-600 dark:text-[#87ff70]" />
              <span class="hidden xs:inline sm:inline">App Tracking</span>
            ...
            <button
              @click="toggleTheme"
              class="flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-600 dark:text-slate-300 transition-all shadow-2xs cursor-pointer active:scale-95 shrink-0"
              :title="isDark ? 'Cambiar a modo claro' : 'Cambiar a modo oscuro'"
              aria-label="Alternar tema"
            >
              <Sun v-if="isDark" class="w-4 h-4 sm:w-5 sm:h-5 text-amber-300 transition-transform rotate-0 hover:rotate-45" />
              <Moon v-else class="w-4 h-4 sm:w-5 sm:h-5 text-indigo-600 transition-transform rotate-0 hover:-rotate-12" />
            </button>
          </div>

        </div>

        <!-- Mobile Navigation Bar (Row 2 on screens < sm) -->
        <div class="sm:hidden pb-2.5 pt-0.5">
          <nav class="grid grid-cols-3 gap-1 p-1 rounded-2xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10">
            <!-- 1. Home -->
            <button
              @click="handleTabClick('home')"
              class="flex items-center justify-center gap-1.5 py-2 px-1.5 rounded-xl text-xs font-semibold transition-all duration-200 cursor-pointer text-center"
              :class="[
                currentTab === 'home'
                  ? 'bg-white dark:bg-white/15 text-slate-900 dark:text-white shadow-xs ring-1 ring-slate-200 dark:ring-white/20 font-bold'
            ...
            <button
              ...
              :class="[
                currentTab === 'utilities'
                  ? 'text-black font-bold shadow-xs'
                  : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              ]"
              :style="currentTab === 'utilities' ? { background: 'var(--kinetic-glow)' } : {}"
            >
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <!-- ============================================================ -->
      <!-- TAB 1: HOME (PRO HUB ECOSYSTEM OVERVIEW)                      -->
      <!-- ============================================================ -->
      <section v-if="currentTab === 'home'" class="space-y-8">
        <!-- Hero Banner -->
        <div class="relative overflow-hidden bg-white dark:bg-white/5 p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-white/10 shadow-sm">
          <div class="relative z-10 max-w-2xl">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-extrabold uppercase mb-3" style="background: rgba(135, 255, 112, 0.15); color: var(--primary); border: 1px solid rgba(135, 255, 112, 0.3);">
              ...
              Ecosistema Profesional BodyFlow
            </div>
            ...
            <p class="mt-2 text-xs sm:text-sm leading-relaxed text-slate-600 dark:text-slate-400">
              Centro de mando para nutrición clínica. Monitorea expedientes de pacientes, diseña menús inteligentes, genera listas de compras automáticas con IA y emite fichas de recomendaciones oficiales.
            </p>
          </div>
        ...
        <!-- Ecosystem Grid: 5 Core Pro Modules -->
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          
          <!-- CARD 1: Directorio & Gestión de Pacientes -->
          <div
            @click="handleTabClick('pacientes')"
            class="bg-white dark:bg-white/5 p-6 rounded-3xl border border-slate-200 dark:border-white/10 hover:border-emerald-500/50 dark:hover:border-emerald-500/40 transition-all duration-300 flex flex-col justify-between group cursor-pointer relative overflow-hidden shadow-sm hover:shadow-md md:col-span-2 lg:col-span-1"
          >
            <div class="space-y-4">
              ...
              <div>
                ...
                <p class="text-xs leading-relaxed mt-2 text-slate-600 dark:text-slate-300">
                  Consulta el historial clínico, notas médicas, planes de alimentación, citas agendadas y monitoreo de adherencia de tus pacientes.
                </p>
              </div>
            </div>

            <div class="mt-6 pt-4 border-t border-slate-100 dark:border-white/5 flex items-center justify-between text-xs font-bold text-emerald-700 dark:text-emerald-400">
              <span class="flex items-center gap-1.5">
                <CheckCircle2 class="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                Abrir Directorio Clínico
              </span>
              <span class="group-hover:translate-x-1 transition-transform">→</span>
            </div>
          </div>

          <!-- CARD 2: Creador de Lista de Compras -->
          <div
          ...
          <!-- CARD 3: Hoja de Recomendaciones -->
          <div
          ...
          <!-- CARD 4: Avances y Progreso Clínico -->
          <div
      ...
      <!-- ============================================================ -->
      <!-- TAB 2: PACIENTES (DIRECTORIO & EXPEDIENTE DE DETALLE)       -->
      <!-- ============================================================ -->
      <section v-else-if="currentTab === 'pacientes'">
